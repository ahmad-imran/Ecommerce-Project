...
def print_invoice(request, order_id):
    ...
    return render(request, 'App_Payment/print-invoice.html', _context)
...
def complete(request):
    ...
    return render(request, "App_Payment/complete.html", context={'orderId': tran_id})
...
def purchase(request, val_id, tran_id):
    ...
    for item in cart_items:
        item.purchased = True
        item.save()
    return HttpResponseRedirect(reverse("App_Payment:order_details", kwargs={'order_id': orderId}))
...
def order_details_view(request, order_id):
    try:
        order = Order.objects.filter(user=request.user, orderId=order_id).first()
        context = {
            "order": order,
            'total_amount': order.get_totals()
        }
        ...
        messages.warning(request, "You do no have an active order")
        return redirect("App_Shop:home")
    return render(request, "App_Payment/order-details.html", context)
